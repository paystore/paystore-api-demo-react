package com.phoebus.appdemo.utils;

public class Constants {
  public static final String PREFS_DATA_PAYMENT = "prefs_data_payment";
  public static final String PREFS_DATA_REVERSE_PAYMENT = "prefs_data_reverse_payment";

  public static final String ERROR = "ERROR_PAYMENT";

  public static final String ON_PAYMENT_EVENT = "onPayment";

  public static final String CANCEL_SIGNATURE = "CANCEL_SIGNATURE";

  public static final String CANCEL_SIGNATURE_ERROR = "CANCEL_SIGNATURE_ERROR";

  public static final String NEW_SIGNATURE = "NEW_SIGNATURE";

  public static final String APPLICATION_ID = "application_id";

  public static final String SECRET_TOKEN = "secret_token";

}
